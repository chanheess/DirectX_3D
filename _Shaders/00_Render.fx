struct VertexMesh   //입력용
{
    ...
    matrix Transform : InstTransform;   //이게 들어가야 월드 변경이 가능
};
...
{
    float depth = input.Position.z / input.Position.w ; //0~1값
    
    //depth = 1.0f - depth * 5.0f; //카메라영역
...
MeshOutput VS_PreRender_Reflection_Mesh(VertexMesh input)    //반사용 매쉬
{
...
MeshOutput VS_PreRender_Reflection_Model(VertexModel input)    //반사용 매쉬
{
...
void SetAnimationWorld(inout matrix world, VertexModel input)
{
    ...
    //Attach Model
    [flatten]
    if (any(input.BlendIndices) == false)   //하나라도 0보다작다면
    {
        indices[0] = BoneIndex;
        weights[0] = 1.0f;
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        c0 = TransformsMap.Load(int4(indices[i] * 4 + 0, currFrame[0], clip[0], 0)); //int4 배열의위치
        ...
        n0 = TransformsMap.Load(int4(indices[i] * 4 + 0, nextFrame[0], clip[0], 0)); //int4 배열의위치
        ...
        currAnim = lerp(curr, next, time[0]); //??????????????lerp는무엇인가
        
        
        [flatten]
        if (clip[1] >= 0)
        {
            c0 = TransformsMap.Load(int4(indices[i] * 4 + 0, currFrame[1], clip[1], 0)); //int4 배열의위치
            ...
            n0 = TransformsMap.Load(int4(indices[i] * 4 + 0, nextFrame[1], clip[1], 0)); //int4 배열의위치
        ...
        transform += mul(weights[i], currAnim); //가중치를 총더해서 이동시킴 (최종행렬)
    }
...
MeshOutput VS_PreRender_Reflection_Animation(VertexModel input)    //반사용 매쉬
{
