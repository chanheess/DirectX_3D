...
float4 PS_Grayscale(VertexOutput input) : SV_Target
{
    ...
    float3 diffuse = Maps[0].Sample(LinearSampler, input.Uv);
...
//0 < Saturation < 1 : desateration
//Saturation = 1 : orignal
...
float4 PS_LensDistortion(VertexOutput input) : SV_Target
{
    ...
    float radiusSquared = aspect * aspect + uv.x * uv.x + uv.xy * uv.y;
