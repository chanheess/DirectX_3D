...
static const float EarthInnerRadius = 6356.7523142f;    //지구의 내부반경
static const float EarthOuterRadius = 6356.7523142f * 1.0157313f;   //지구의 외부반경
...
static const float EarthScale = 1.0 / (6356.7523142 * 1.0157313 - 6356.7523142);    // 1 / 지구의 크기
...
float HitOuterSphere(float3 position, float3 direction) //카메라밖의 충돌하는 a와 C지점 계산
{
...
float2 GetDensityRatio(float height)    //높이 고도에 대한 비율
{
...
float2 GetDistance(float3 p1, float3 p2)    //태양과의 거리
{
...
VertexOutput_Dome VS_Dome(VertexTexture input)
{
    ...
    output.oPosition = -input.Position.xyz; //뒤집은 이유 위에서 밑으로의 좌표라
