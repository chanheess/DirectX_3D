...
Texture2D ShadowMap;    //깊이했던 텍스쳐가 들어옴
SamplerComparisonState ShadowSampler;   //cpp에서 만들어서 보냄, 비교연산이 들어감
...
struct MeshOutput
{
    ...
    //픽셀쉐이더에서 쓰려고 SV_Position과 동일한 값을 넣어줌 
    //SV_붙은것은 픽셀쉐이더에서 못씀
    ...
    float4 sPosition : Position5; //Shadow VP Position
    float4 gPosition : Position6; //Geometry Position
    
    float3 Normal : Normal;
    float3 Tangent : Targent;
    float2 Uv : Uv;
...
struct MeshGeometryOutput
{
    ...
    uint TargetIndex : SV_RenderTargetArrayIndex; //geometry에서만 사용가능/ 몇번 array를 사용할것인가
};
