...
cbuffer CB_Water
{
    ...
    float WaveScale = 0.05f; //물결의 크기

    float WaterShininess = 30.0f; //해가 비침
    float WaterAlpha = 0.5f; //물의 알파값
    float WaveSpeed = 0.06f; //물의 속도
};
...
float4 PS_Water(VertexOutput_Water input) : SV_Target
{
    ...
    float4 refractionColor = saturate(ReflectionMap.Sample(LinearSampler, refraction) + RefractionColor);   //물의색
    ...
    float r = (1.2 - 1.0f) / (1.2f / 1.0f); //매질
    float fresnel = saturate(min(1, r + (1 - r) * pow(1 - dot(normal, heightView), 2)));    //강도
    float3 diffuse = lerp(reflectionColor, refractionColor, fresnel);
    
    float3 R = normalize(reflect(light, normal)); //specular
