...
struct ParticleDesc
{
    ...
    float CurrentTime;  //정점의 플레이 되고있는 시간
};
...
float4 ComputePosition(float3 position, float3 velocity, float age, float normalizedAge)
{
    ...
    float velocityIntelgral = startVelocity * normalizedAge + (endVelocity - startVelocity) * normalizedAge / 2;
    
    position += normalize(velocity) * velocityIntelgral * Particle.Duration;
...
VertexOutput VS(VertexInput input)
{
    ...
    float age = Particle.CurrentTime - input.Time;  //첫번째 입자가 나오고 그 다음까지의 시간
...
BlendState NonPremultiplied_Particle
{
