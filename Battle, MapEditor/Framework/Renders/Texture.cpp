...
void Texture::ReadPixel(DXGI_FORMAT readFormat, float* heightColors)
{
	ID3D11Texture2D* srcTexture;
	view->GetResource((ID3D11Resource **)&srcTexture);
	D3D11_TEXTURE2D_DESC srcDesc;
	...
	{
		memcpy(heightColors, subResource.pData, sizeof(float) * desc.Width * desc.Height);
	}
...
void Texture::CreateWritingTexture(DXGI_FORMAT readFormat, UINT num)
{
	ID3D11Texture2D* srcTexture = this->GetTexture();
	...
	{
	case 0:
		writingTexture = texture;
		break;
	case 1:
		writingTexture2 = texture;
		break;
	}
}

void Texture::CreateSRVTexture(DXGI_FORMAT readFormat, UINT num)
{
	ID3D11Texture2D* srcTexture = this->GetTexture();
	...
	{
	case 0:
		Check(D3D::GetDevice()->CreateTexture2D(&desc, NULL, &textureSRV));
		break;
	case 1:	
		Check(D3D::GetDevice()->CreateTexture2D(&desc, NULL, &textureSRV2));
		break;
	}
}

void Texture::ChangeSRVTexture(DXGI_FORMAT readFormat, ID3D11Texture2D* texture, UINT num)
{
	switch (num)
	{
	case 0:
		Check(D3DX11LoadTextureFromTexture(D3D::GetDC(), texture, NULL, textureSRV));
		break;
	case 1:
		Check(D3DX11LoadTextureFromTexture(D3D::GetDC(), texture, NULL, textureSRV2));
		break;
	}
	...
	{
	case 0:
		Check(D3D::GetDevice()->CreateShaderResourceView(textureSRV, &desc, &view));
		break;
	case 1:
		Check(D3D::GetDevice()->CreateShaderResourceView(textureSRV2, &desc, &view));
		break;
	}

}

void Texture::ReadPixel2(DXGI_FORMAT readFormat, vector<Color>* pixels)
{
	ID3D11Texture2D* srcTexture;
	view->GetResource((ID3D11Resource **)&srcTexture);
	D3D11_TEXTURE2D_DESC srcDesc;
