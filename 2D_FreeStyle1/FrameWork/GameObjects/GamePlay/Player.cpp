...
#include "./GameObjects/GamePlay/p1color.h"
#include "./Components/Bounding/BBox.h"
#include "./Components./Etc/Audio.h"

Player::Player()
	: GameObject(L"player"), diago(false), handBall(false), shooting(false)
...
void Player::Update()
{
	...
	p1->transform->Position(objXPos);
	p1->Update();
	if(breaker == false)
		FixPos = objX->transform->Position();
	//모든 동작을할때 첫시작위치를 잡고 
	//플레이어는 움직인뒤 다시 플레이어와
	//위치 똑같이하기로 하여 위치오류 없애기
	...
	{
		//서있기
		if (Input::Get()->KeyUp(VK_LEFT))
		{
			transform->Scale(adjScale.x, adjScale.y);
			anim->Play(L"standleft");
			diago = false;
		}
		if (Input::Get()->KeyUp(VK_RIGHT))
		{
			transform->Scale(adjScale.x, adjScale.y);
			anim->Play(L"standright");
			diago = false;
		}
		if (Input::Get()->KeyUp(VK_DOWN))
		{
			transform->Scale(adjScale.x, adjScale.y);
			anim->Play(L"standdown");
			diago = false;
		}
		if (Input::Get()->KeyUp(VK_UP))
		{
			transform->Scale(adjScale.x, adjScale.y);
			anim->Play(L"standup");
			diago = false;
		...
		//대각선 이동
		if (Input::Get()->Key(VK_RIGHT) &&
			(Input::Get()->Key(VK_DOWN)))
		{
		...
		else if (Input::Get()->Key(VK_RIGHT) &&
			(Input::Get()->Key(VK_UP)))
		{
			transform->Scale(adjScale.x + 25.0f, adjScale.y);
			anim->Play(L"mru");
...
void Player::Render()
{
	...
	/*ImGui::Begin("Debug");
	{
		ImGui::Text("p1Distince : %.2f", (D3DXVec3Length(&(objXPos - goalPos))));
		ImGui::Text("p1animSpeed : %.2f", speed);
		ImGui::Text("p1breakth : %d", breakth);
		ImGui::Text("p1ScaleX : %.2f p1ScaleY : %.2f", scaleSave.x, scaleSave.y);
	}
	ImGui::End();*/
}

void Player::JumpControl(int frame)
{
	if(dontmove == false)
	{
...
void Player::JumpShoot(int pos)
{
	...
	//슛페이크
	if (Input::Get()->KeyUp('P') && noStop == false)
	{
	...
	//피벗	----	애니메이션 따야됨^^
	/*if (Input::Get()->KeyDown(VK_DOWN) && noMove == 2)
	{
		
	}*/
	//슛
	if (Input::Get()->Key('P') || noStop == true)
	{
...
void Player::Block()
{
	if (Input::Get()->KeyDown('P') && block == false && dontmove == false)
	{
		if (Input::Get()->Key(VK_DOWN)) //중앙
		{
			blockDirection = 1;
			objxFix = true;	// x충돌체 고정
			block = true;
		...
		if (Input::Get()->Key(VK_RIGHT) && Input::Get()->Key(VK_DOWN)) //오른쪽 대각선
		{
			blockDirection = 2;
			objxFix = true;	// x충돌체 고정
			block = true;
		...
		else if (Input::Get()->Key(VK_RIGHT)) //오른쪽
		{
			blockDirection = 4;
			objxFix = true;	// x충돌체 고정
			block = true;
		...
		if (Input::Get()->Key(VK_LEFT) && Input::Get()->Key(VK_DOWN)) //왼쪽 대각선
		{
			blockDirection = 3;
			objxFix = true;	// x충돌체 고정
			block = true;
		...
		else if (Input::Get()->Key(VK_LEFT)) //왼쪽
		{
			blockDirection = 5;
			objxFix = true;	// x충돌체 고정
			block = true;
...
void Player::LayupDunk(int pos)
{
	...
	//레이업슛
	if (dontmove == false && (Input::Get()->KeyDown('P') || LayOrDunk == 1))
	{
		if (LeRiPos() == 1 && posShoot == 1 && Input::Get()->Key(VK_RIGHT)) //사이드
		{
			if (noChange[0] == true)
			{
				objxFix = true;	// x충돌체 고정
		...
		else if (LeRiPos() == 2 && posShoot == 1 && Input::Get()->Key(VK_LEFT))
		{
			if (noChange[1] == true)
			{
				objxFix = true;	// x충돌체 고정
		...
		if (LeRiPos() == 1 && posShoot == 2 && Input::Get()->Key(VK_RIGHT) 
			&& Input::Get()->Key(VK_UP)) //대각
		{
			if (noChange[2] == true)
			{
				objxFix = true;	// x충돌체 고정
		...
		else if (LeRiPos() == 2 && posShoot == 2 && Input::Get()->Key(VK_LEFT) 
			&& Input::Get()->Key(VK_UP))
		{
			if (noChange[3] == true)
			{
		...
		if (LeRiPos() == 3 && posShoot == 3 && Input::Get()->Key(VK_UP)) //중앙
		{
			if (noChange[4] == true)
			{
				objxFix = true;	// x충돌체 고정
	...
	//덩크슛
	if (dontmove == false && (Input::Get()->KeyDown('O') || LayOrDunk == 2))
	{
		if (LeRiPos() == 1 && posShoot == 1 && Input::Get()->Key(VK_RIGHT)) //사이드
		{
			if (noChange[0] == true)
...
void Player::Breakthrough()
{
	...
	//돌파
	if (dontmove == false && Input::Get()->KeyDown('I') && breaker == false)
	{
		if (LeRiPos() == 1 && posShoot == 1) //사이드
		{
			if (Input::Get()->Key(VK_UP))
			{
		...
		if (LeRiPos() == 1 && posShoot == 2) //대각
		{
			if (Input::Get()->Key(VK_LEFT) || Input::Get()->Key(VK_UP))
			{
		...
		else if (LeRiPos() == 2 && posShoot == 2)
		{
			...
			if (Input::Get()->Key(VK_RIGHT) || Input::Get()->Key(VK_UP))
			{
				transform->Scale(adjScale.x + 50.0f, adjScale.y + 10.0f);
				anim->Play(L"rdrbt");
				if (breakth != 10)
...
void Player::Stopbreakth()
{
	anim->Stop();
}

Vector3 Player::Shoot()
{
