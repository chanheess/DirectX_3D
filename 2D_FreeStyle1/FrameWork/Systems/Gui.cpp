...
Gui::Gui()
{
	...
	ImGui_ImplDX11_Init(D2D::GetDevice(), D2D::GetDC());
...
void Gui::ApplyStyle()
{
	...
	ImVec4 backgroundLight = ImVec4(0.37f, 0.3f, 0.39f, 1.0f);
...
void Gui::DockingPannel()
{
	...
	ImGui::Begin("DockingPannel", &bDocking, windowFlags);
	{
		...
		if (io.ConfigFlags & ImGuiConfigFlags_DockingEnable)
		{
			...
			ImGui::DockSpace(pannelID, ImVec2(0, 0),ImGuiDockNodeFlags_PassthruDockspace);
		}
	}
	ImGui::End();
}
...
void Gui::RenderText(float x, float y, string context)
{
	GuiText text;
...
void Gui::RenderText(float x, float y, float r, float g, float b, string context)
{
	GuiText text;
	text.Position = D3DXVECTOR2(x, y);
...
void Gui::RenderText(D3DXVECTOR2 position, D3DXCOLOR color, string context)
{
	GuiText text;
